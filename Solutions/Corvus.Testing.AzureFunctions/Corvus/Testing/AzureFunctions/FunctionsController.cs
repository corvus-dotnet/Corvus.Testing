...
StdErr: {StdErr}",
        ...
        private async Task<string> GetToolPath()
        {
            ...
            Console.WriteLine($"{p.Id}:{Environment.CurrentManagedThreadId} ({p.ProcessName}, {Environment.CommandLine}) " + string.Join(", ", toolPaths.Select(p => $"'{p}'")));
            Console.WriteLine($"{p.Id}:{Environment.CurrentManagedThreadId} PATH: {Environment.GetEnvironmentVariable("PATH")}");
            ...
            foreach (string toolPath in toolPaths)
            {
                ...
                Console.WriteLine($"{p.Id}:{Environment.CurrentManagedThreadId} Testing tool path '{toolPath}' can be used for running Functions projects.");
                ...
                try
                {
                    ...
                    if (await process.ExitCode == 0)
                    {
                        ...
                        Console.WriteLine($"{p.Id}:{Environment.CurrentManagedThreadId} Using '{toolPath}'");
                        return toolPath;
                    }
                }
                catch (Win32Exception ex) when (ex.NativeErrorCode == INVALID_EXECUTABLE)
                {
                    ...
                    Console.WriteLine($"{p.Id}:{Environment.CurrentManagedThreadId} '{toolPath}' INVALID_EXECUTABLE");
                    continue;
                }
            }
