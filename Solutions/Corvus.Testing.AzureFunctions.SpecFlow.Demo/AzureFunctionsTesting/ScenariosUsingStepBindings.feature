...
Scenario: A Get request including a name in the querystring is successful
	Given I start a functions instance for the local project 'Corvus.Testing.SpecFlow.DemoFunction' on port 7075 with runtime 'netcoreapp3.1'
...
Scenario: Supplying an alternative greeting via configuration
	...
	And I start a functions instance for the local project 'Corvus.Testing.SpecFlow.DemoFunction' on port 7075 with runtime 'netcoreapp3.1'
	When I send a GET request to 'http://localhost:7075/?name=Jon'
	Then I receive a 200 response code
